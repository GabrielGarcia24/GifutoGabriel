				...
				<div class="col-sm-9 padding-right">
					<div class="features_items"><!--features_items-->
						<h2 class="title text-center">Productos destacados</h2>
                                                ...
                                                <%  
                                                    int idCategory=Integer.parseInt(session.getAttribute("category").toString());
                                                    
                                                    if(idCategory>0){
                                                        datos=ProductoDAO.listarProductosPorCategoria(idCategory);
                                                    }else{
                                                        datos=ProductoDAO.listarProductosRecomendados();
                                                    }
							...
							<div class="product-image-wrapper">
								<div class="single-products">
										<div class="productinfo text-center">
                                                                                    <img style=" width:241px; height:225px;" src="foto/<%=p.getFoto() %>" alt="" />
										
                                                                                    <h2 <% if(p.getExistencia()==0){ %> class="gris"<% } %>>$<%=p.getCosto() %></h2>
											<p><%=p.getNombre() %></p>
											<span class="btn btn-default add-to-cart  <% if(p.getExistencia()==0){ %> disabled <% } %> "><i class="fa fa-shopping-cart"></i>Agregar al carrito</span>
										</div>
										<div class="product-overlay <% if(p.getExistencia()==0){ %> grisfondo <% } %> ">
											<div class="overlay-content">
												<h2>$<%=p.getCosto() %></h2>
												<p><%=p.getNombre() %></p>
												<a href="Carrito?id=&action=ordenar" class="btn btn-default add-to-cart  <% if(p.getExistencia()==0){ %> disabled <% } %>  "><i class="fa fa-shopping-cart"></i>Agregar al carrito</a>
											</div>
										</div>
								<% if(p.isNuevo()){ %>
								...
								<div class="choose">
									<ul class="nav nav-pills nav-justified">
										<li><a   <% if(p.getExistencia()==0){ %> class="gris"  <% } %>>
                                                                                        <% if(p.getExistencia()>0){ %>  
                                                                                       <i class="fa fa-check-circle"></i> <%="("+p.getExistencia()+")" %> Disponible
                                                                                       <% }else{ %>
                                                                                       <i class="fa fa-lock"></i><%="("+p.getExistencia()+")" %>Agotado 
                                                                                        <% } %>
                                                                                    </a></li>
									</ul>
								</div>
